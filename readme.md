...
| 2 | Replace all four stock oxygen sensors with one post-turbo wideband feedback source. | *Integrated development patch built* — the AEM X-Series 30-0300 signal enters through the former MAF ADC, feeds both stock bank lambda/readiness paths, and is directly loggable. Both front A/F and both rear O2 conversion/monitor paths plus 18 mapped DTC switches are removed. The older one-factory-front-sensor patch remains only as a standalone historical alternative. Heater drivers are not electrically forced off. See [master_patch/README.md](master_patch/README.md). |
| 3 | Repurpose removed sensor inputs for other hardware. | **Architecture decided** — the former MAF input is the AEM wideband channel. Original oxygen-sensor circuits are deliberately not repurposed; all four connectors must be disconnected and insulated. |
| 4 | Reuse the EVAP solenoid as an EBCS solenoid + WRX-style boost control. | *Integrated development patch built* — the master keeps the verified proportional + feed-forward controller, throttle gate, AEM-ready gate, MAP/RPM/IAT validity gates, minimum-control-RPM and SD-fault gates, 5.5 psi soft duty shutdown, 6.5 psi hard fuel cut, and runtime boost-enable switch. It uses the exact Omni MAP-SUP-3BR transfer and commands zero base duty, zero Kp, and zero maximum duty for initial 5 psi spring-only operation. Output is ATU-II `0xFFFFF590`; feedback is native MAP at `0xFFFFABC4`. Static/binary verified, not bench or vehicle verified. |
| 5 | Replace MAF logic with MAFless Speed Density. | *Integrated development patch built* — final airflow is an always-on RomRaider-editable 13x17 MAP/RPM VE model with retained post-intercooler IAT correction and no MAF fallback. Raw MAF conversion/filter/diagnostic paths and P0102/P0103 are bypassed; the ADC remains live for the AEM input. Invalid running data selects a fixed 500 g/s high-load fail-safe; exact zero RPM writes zero. See [speed_density/README.md](speed_density/README.md) for the component and [master_patch/README.md](master_patch/README.md) for the integrated image. |
...
| 7 | Produce one focused turbo-conversion master image and definition. | **Development baseline complete** — `master_patch` deterministically composes SD, exact 3-bar MAP scaling, boost safeties, AEM input/four-stock-O2 delete, STI-pink donor scaling/deadtime, base VE, conservative fuel/timing, early AVLS, and 6800 RPM limit from immutable stock. Output, definition, logger fragment, and Ghidra audit pass static verification; physical commissioning remains. |
...
| [speed_density/README.md](speed_density/README.md) | Separate always-on MAFless MAP/RPM/IAT VE-model patch, Ghidra trace, calibration layout, verifier, and commissioning boundary. |
...
| [speed_density/D2WD610H_AVLS_speed_density_patch.xml](speed_density/D2WD610H_AVLS_speed_density_patch.xml) | Generated self-contained metric RomRaider definition: D2WD610H standard tables + AVLS + standalone MAFless speed-density VE/IAT calibration, with inherited MAF tables/DTCs removed. |
| [master_patch/D2WD610H_master_patch.xml](master_patch/D2WD610H_master_patch.xml) | Focused self-contained metric master definition: active timing/KCA identities, fuel/injectors, AVLS, SD/VE, exact Omni MAP, boost, and AEM transfer/range; obsolete MAF/O2/diagnostic and dormant timing entries removed. |
