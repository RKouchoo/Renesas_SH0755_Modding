...
| 5 | Replace MAF logic with MAFless Speed Density. | *Standalone development patch built* — the stock task is retained for downstream load/filter state, but its final airflow calculation is replaced by an always-on RomRaider-editable 13x17 MAP/RPM VE model with post-intercooler IAT correction and no MAF fallback. Both raw conversion calls, the stock raw-MAF limit/filter update, the high/low diagnostic task, and the MAF-dependent temperature condition are bypassed; P0102/P0103 are disabled. Invalid running-state data selects a fixed 500 g/s rich/high-load fail-safe; exact zero RPM writes zero. It is independently binary-verified and non-overlapping, but is not in the combined patch/base turbo map and is not vehicle-validated. See [speed_density/README.md](speed_density/README.md). |
...
> Load exactly **one** of the six custom RomRaider definitions at a time—the AVLS-only,
> boost-patch, single-front-A/F-patch, rotational-idle-patch, speed-density-patch, or combined variant matching the ROM being edited. Each embeds a metric
