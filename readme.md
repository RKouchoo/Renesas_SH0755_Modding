...
| 5 | Potentially change MAF logic to Speed Density. | *Standalone development patch built* — a default-OFF wrapper runs the complete stock airflow task first, then uses a RomRaider-editable 13x17 VE surface, absolute MAP, RPM, post-intercooler IAT correction, displacement, and guarded scalars to replace the final stock mass-airflow channel. Invalid input retains stock MAF behavior. It is independently binary-verified and non-overlapping, but is not in the combined patch/base turbo map and is not vehicle-validated. See [speed_density/README.md](speed_density/README.md). |
...
| [speed_density/README.md](speed_density/README.md) | Separate default-OFF MAP/RPM/IAT VE-model patch, Ghidra trace, calibration layout, verifier, and commissioning boundary. |
...
| [speed_density/D2WD610H_AVLS_speed_density_patch.xml](speed_density/D2WD610H_AVLS_speed_density_patch.xml) | Generated self-contained metric RomRaider definition: D2WD610H standard tables + AVLS + standalone speed-density VE/IAT calibration and default-OFF switch. |
