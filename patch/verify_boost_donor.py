...
def main():
    ...
    assert list(patched[0x7D7C4:0x7D7CC]) == expected_base
    stored_target = floats(patched, 0x7D7E0, 8)
    expected_target = [ATM_NATIVE + psi * NATIVE_PER_PSI for psi in expected_target_psi]
    close(stored_target, expected_target)
    close(floats(patched, 0x7D800, 3), [0.0005, 0.33, ATM_NATIVE + 6 * NATIVE_PER_PSI])
    close(floats(patched, 0x7D8A4, 2), [30.0, ATM_NATIVE + 7 * NATIVE_PER_PSI])
    assert floats(patched, 0x72810, 2) == floats(donor, 0xC00C0, 2)
