...
Usage: python3 tests/verify_boost_donor.py [donor.hex] [patched.bin]
"""
...
PATCHED = Path(sys.argv[2]) if len(sys.argv) > 2 else ROOT / "patches/core/D2WD610H_boost.bin"
if len(sys.argv) > 3:
    raise SystemExit("usage: python3 tests/verify_boost_donor.py [donor.hex] [patched.bin]")
...
def main():
    ...
    expect(patched, 0x7D8C4, bytes.fromhex("4f22d30d430be410d20c420b2f06"),
           "native lock then limiter; preserve prior mask in delay slot")
    expect(patched, 0x7D8D2,
           bytes.fromhex("d10c601088018b0c"),
           "added-fuel-cut disabled branch")
    expect(patched, 0x7D8E2, bytes.fromhex("f2358b06d10a6010cb802100d109e0ff2101"),
           "pressure comparison; publish BF6C and native B744 inhibition")
    expect(patched, 0x7D8F4, bytes.fromhex("64f6d308432b4f26"),
           "restore prior mask through native unlock with caller PR")
    expect(patched, 0x7D8FC,
    ...
    decoded = decode_span(patched, patch.REVWRAP_ADDR, 0x7D8FC)
