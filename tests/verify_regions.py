#!/usr/bin/env python3
"""Inspect saved-ROM literal patterns and known occupied RAM intervals.

- Scans mov.w / mov.l-shaped words for PC-relative RAM literals. Scanning data
  as instructions can yield false positives; computed accesses can be missed.
- Adds known injector records and state/stack intervals. Gaps in this scan do
  not establish available scratch RAM.
...
HERE  = str(_test_paths.ROOT / "patches/core")
BIN   = sys.argv[1] if len(sys.argv) > 1 else str(_test_paths.ROOT / "2005 BLE MT.bin")
...
computed = [("injector control records", SOL_BASE, SOL_END),
...
print("distinct candidate RAM literals: %d" % len(ram_refs))
print("injector control records: 0x%08X .. 0x%08X (base literal matches %d times as 0x%X)"
...
# ---- 3. inventory literal-match gaps; no allocation claim follows ----
print("\n=== literal-match gaps below 0x%X; NOT proof of unused RAM ===" % SOL_BASE)
refs = sorted(a for a in ram_refs if 0xFFFF6000 <= a < SOL_BASE)
best = None
