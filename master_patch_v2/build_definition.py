...
SD_SOURCE = ROOT / "patches/speed_density" / "D2WD610H_AVLS_speed_density_patch.xml"
BOOST_SOURCE = ROOT / "defs" / "D2WD610H_AVLS_boost_patch.xml"
OUTPUT = HERE / "D2WD610H_master_patch_v2.xml"
...
REPAIR_NAMES = (
    "Deceleration Dashpot Air Decrement",
    "Transient Fuel Falling Load Filter Response",
    "Coolant Final Timing Minimum",
    "RPM Final Timing Minimum",
)
...
TABLE_CATEGORY_OVERRIDES = {
    ...
    "Knock Correction Advance Max A (Normal Cam)": CAT_IGN_KNOCK,
    "Knock Correction Advance Max B (AVLS High Cam)": CAT_IGN_KNOCK,
    ...
    "Deceleration Dashpot Air Decrement": CAT_IDLE_TARGET,
    "Transient Fuel Falling Load Filter Response": CAT_FUEL_LEARNING,
    "Coolant Final Timing Minimum": CAT_IGN_COMP,
    "RPM Final Timing Minimum": CAT_IGN_COMP,
    "Base Timing A (Normal Cam, AVCS 1.0)": CAT_IGN_BASE,
    "Base Timing D (Normal Cam, AVCS 0.0)": CAT_IGN_BASE,
    "Base Timing C (AVLS High Cam, AVCS 1.0)": CAT_IGN_BASE,
    "Base Timing F (AVLS High Cam, AVCS 0.0)": CAT_IGN_BASE,
}
...
TIMING_RENAMES = {
    "Base Timing A": "Base Timing A (Normal Cam, AVCS 1.0)",
    "Base Timing C": "Base Timing C (AVLS High Cam, AVCS 1.0)",
    "Base Timing D": "Base Timing D (Normal Cam, AVCS 0.0)",
    "Base Timing F": "Base Timing F (AVLS High Cam, AVCS 0.0)",
    "Knock Correction Advance Max A": "Knock Correction Advance Max A (Normal Cam)",
    "Knock Correction Advance Max B": "Knock Correction Advance Max B (AVLS High Cam)",
}

AVCS_RENAMES = {
    "Intake Cam Advance Angle A (AVCS)": "Intake AVCS Target A (AVLS Low Cam)",
    "Intake Cam Advance Angle B (AVCS)": "Intake AVCS Target B (AVLS High Cam)",
}
...
TIMING_DESCRIPTIONS = {
    "Base Timing A (Normal Cam, AVCS 1.0)": (
    ...
    "Base Timing D (Normal Cam, AVCS 0.0)": (
        "Normal/AVLS-low-cam base timing for an intake-AVCS tracking ratio of "
    ...
    "Base Timing C (AVLS High Cam, AVCS 1.0)": (
    ...
    "Base Timing F (AVLS High Cam, AVCS 0.0)": (
    ...
    "Knock Correction Advance Max A (Normal Cam)": (
    ...
    "Knock Correction Advance Max B (AVLS High Cam)": (
...
AVCS_DESCRIPTIONS = {
    "Intake AVCS Target A (AVLS Low Cam)": (
    ...
    "Intake AVCS Target B (AVLS High Cam)": (
        "Intake-cam advance target, in degrees, selected while committed AVLS "
...
def add_timing_minimum_templates(parent: ET.Element, target: ET.Element) -> None:
    # RPM Final Timing Minimum (0x5FB78 descriptor, 0x78240 data, 0x78220 axis)
    table_rpm = ET.SubElement(
        parent,
        "table",
        {
            "type": "2D",
            "name": "RPM Final Timing Minimum",
            "category": CAT_IGN_COMP,
            "storagetype": "uint8",
            "endian": "big",
            "sizey": "8",
            "userlevel": "3",
    ...
    ET.SubElement(
        ...
        "scaling",
        {
            ...
            "to_byte": "(x+20)/.3515625",
            "format": "0.00",
            "fineincrement": ".2",
            "coarseincrement": "1",
        },
    )
    axis_rpm = ET.SubElement(
        table_rpm,
        "table",
        {
            "type": "Y Axis",
    ...
    ET.SubElement(
        axis_rpm,
        "scaling",
        {"units": "RPM", "expression": "x", "to_byte": "x", "format": "#", "fineincrement": "50", "coarseincrement": "100"},
    )
    set_description(
        table_rpm,
        "Engine-speed-dependent lower bound on final ignition timing. Firmware selects the greater of this RPM table and the Coolant Final Timing Minimum table.",
    )
    t_rpm = ET.SubElement(target, "table", {"name": "RPM Final Timing Minimum", "storageaddress": "0x78240"})
    ET.SubElement(t_rpm, "table", {"type": "Y Axis", "storageaddress": "0x78220"})

    # Coolant Final Timing Minimum (0x5FB8C descriptor, 0x78268 data, 0x78248 axis)
    table_ect = ET.SubElement(
        parent,
        "table",
        {
            "type": "2D",
            "name": "Coolant Final Timing Minimum",
            "category": CAT_IGN_COMP,
            "storagetype": "uint8",
            "endian": "big",
            "sizey": "8",
    ...
    ET.SubElement(
        axis_ect,
        "scaling",
        {"units": "Degrees C", "expression": "x", "to_byte": "x", "format": "#", "fineincrement": "1", "coarseincrement": "5"},
    )
    set_description(
        table_ect,
        "Coolant-temperature-dependent lower bound on final ignition timing. Stock drops to 0 deg above 40 C; this floor prevents spark advance from going negative under extreme retard conditions.",
    )
    t_ect = ET.SubElement(target, "table", {"name": "Coolant Final Timing Minimum", "storageaddress": "0x78268"})
    ET.SubElement(t_ect, "table", {"type": "Y Axis", "storageaddress": "0x78248"})


def add_repair_calibration_tables(parent: ET.Element, target: ET.Element) -> None:
    add_scalar_template(
        parent,
        target,
        "Deceleration Dashpot Air Decrement",
        "0x7963C",
        "float",
        "Air Decrement per Task Call",
        "x",
        "x",
        "Decel Air Decrement",
        "Deceleration air decrement per 8ms task update. Smaller values extend the dashpot air decay window on throttle closure to gently float RPM down to idle without undershooting or stalling. Master v2 default is 0.15 (stock was 0.6).",
        fmt="0.000",
        fine=".01",
        coarse=".05",
    )
    add_scalar_template(
        ...
        "0x76050",
        "float",
        "Filter Response Rate (fraction)",
        "x",
        "x",
        "Filter Response",
        "Filter response rate (fraction per update) for following falling engine load in the transient fueling calculation (B874). Higher values (master v2 default 0.08, stock was 0.01) allow the followed load memory to catch up quickly on decel, clearing negative transient fuel subtraction and preventing low-RPM fuel starvation.",
        fmt="0.000",
        fine=".01",
        coarse=".05",
    )
...
def validate(root: ET.Element) -> None:
    ...
    expected_categories = {
        ...
        "Intake AVCS Target A (AVLS Low Cam)": CAT_CAM_AVCS,
        "Overboost Fuel Cut Enable": CAT_BOOST_PROTECTION,
    ...
    expected_addresses = {
        "Base Timing A (Normal Cam, AVCS 1.0)": "0x78AA0",
        "Base Timing C (AVLS High Cam, AVCS 1.0)": "0x78CD0",
        "Base Timing D (Normal Cam, AVCS 0.0)": "0x78E34",
        "Base Timing F (AVLS High Cam, AVCS 0.0)": "0x79064",
        "Knock Correction Advance Max A (Normal Cam)": "0x7924C",
        "Knock Correction Advance Max B (AVLS High Cam)": "0x793AC",
        "Intake AVCS Target A (AVLS Low Cam)": "0x7C5B0",
        "Intake AVCS Target B (AVLS High Cam)": "0x7C764",
